...
exports.GAME = {
    ...
    processCommand: function (text, update, env, sendMessage, startTime) {
        var _a, _b, _c, _d, _e;
        return __awaiter(this, void 0, void 0, function () {
            var chatId, userId, firstName, args, botUsername, replyMarkup, msg, user, status, profileText, currentTime, marketText, _i, _f, _g, itemKey, data, currentPrice, investment, user, result, netChange, newBalance, investText, invItems, invText, _h, invItems_1, row, itemData, emoji, nameDisplay, user, reviveCost, newBalance, msg, targetId, targetName, caller, target, currentSeconds, roll, STANDARD_DROPS, dropItem, itemDisplay, dropEmoji, targetId, targetName, robber, target, currentSeconds, roll, stealPercent, stolenAmount, fineAmount, itemToSell_1, itemData, invItems, userItem, user, currentTime, sellPrice, topPlayers, lbText, rank, _j, topPlayers_1, p, medal, user, playerLevel, hit, bossDefense, raidText, reward, idText, replyId, replyName, latency, targetId, targetName, user, amount, targetId, targetName, target, newBalance, action, symbol, amount, targetId, targetName, sender, target, user, currentSeconds, remaining, plan, cost, days, newProtectionTime, e_1, adminData, isOwner, isHighAdmin, targetId, level, idArg, lvlArg, targetId, idArg, targetId, titleName, idArg, e_2, allAdmins, list, _k, allAdmins_1, a;
            return __generator(this, function (_l) {
                switch (_l.label) {
                    case 0:
                        chatId = update.message.chat.id;
                        userId = update.message.from.id;
                        firstName = update.message.from.first_name || "Agent";
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, sendMessage(msg, replyMarkup)];
                    case 2:
                        // Notice we are passing replyMarkup as the second argument now
                        _l.sent();
                        return [2 /*return*/];
                    case 3:
                        if (!(text.startsWith("/profile") || text.startsWith("/me"))) return [3 /*break*/, 6];
                    ...
                    case 5:
                        ...
                        return [2 /*return*/];
                    case 6:
                        if (!text.startsWith("/market")) return [3 /*break*/, 8];
                        ...
                        for (_i = 0, _f = Object.entries(config_1.MARKET_ITEMS); _i < _f.length; _i++) {
                            ...
                            currentPrice = muscle_1.MUSCLE.calculateMarketPrice(data.base_price, data.volatility, currentTime);
                    ...
                    case 7:
                        ...
                        return [2 /*return*/];
                    case 8:
                        if (!text.startsWith("/invest")) return [3 /*break*/, 18];
                    ...
                    case 10:
                        ...
                        if (!(isNaN(investment) || investment <= 0)) return [3 /*break*/, 12];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Sahi amount daal bhai."))];
                    case 11:
                        _l.sent();
                        return [2 /*return*/];
                    case 12: return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 13:
                        user = _l.sent();
                        if (!(!user || user.balance < investment)) return [3 /*break*/, 15];
                    ...
                    case 17:
                        ...
                        return [2 /*return*/];
                    case 18:
                        if (!text.startsWith("/inv")) return [3 /*break*/, 21];
                    ...
                    case 20:
                        ...
                        return [2 /*return*/];
                    case 21:
                        if (!(text === "/revive" || text === "/heal")) return [3 /*break*/, 28];
                        return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 22:
                        user = _l.sent();
                        if (!user)
                            return [2 /*return*/];
                        if (!(user.is_alive === 1)) return [3 /*break*/, 24];
                    ...
                    case 27:
                        ...
                        return [2 /*return*/];
                    case 28:
                        if (!(text.startsWith("/kill") || text.startsWith("/k "))) return [3 /*break*/, 51];
                        if (!!update.message.reply_to_message) return [3 /*break*/, 30];
                    ...
                    case 30:
                        ...
                        if (!(userId === targetId)) return [3 /*break*/, 32];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Khud ko kyu maar raha hai bhai?"))];
                    case 31:
                        _l.sent();
                        return [2 /*return*/];
                    case 32:
                        if (!update.message.reply_to_message.from.is_bot) return [3 /*break*/, 34];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Bot pe goli nahi chalti."))];
                    case 33:
                        _l.sent();
                        return [2 /*return*/];
                    case 34: return [4 /*yield*/, db_1.DB_MANAGER.ensureUserExists(env.DB, targetId)];
                    case 35:
                        _l.sent();
                        return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 36:
                        caller = _l.sent();
                        if (!(!caller || caller.is_alive === 0)) return [3 /*break*/, 38];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.dead, " Tu pehle hi mar chuka hai! Use /revive first."))];
                    case 37:
                        _l.sent();
                        return [2 /*return*/];
                    case 38: return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, targetId)];
                    case 39:
                        target = _l.sent();
                        if (!(!target || target.is_alive === 0)) return [3 /*break*/, 41];
                    ...
                    case 41:
                        ...
                        if (!(target.protection_until && target.protection_until > currentSeconds)) return [3 /*break*/, 43];
                    ...
                    case 51:
                        if (!text.startsWith("/rob")) return [3 /*break*/, 77];
                        if (!!update.message.reply_to_message) return [3 /*break*/, 53];
                    ...
                    case 53:
                        ...
                        if (userId === targetId)
                            return [2 /*return*/];
                        if (!update.message.reply_to_message.from.is_bot) return [3 /*break*/, 55];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Bot ki jeb khaali hoti hai."))];
                    case 54:
                        _l.sent();
                        return [2 /*return*/];
                    case 55: return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 56:
                        robber = _l.sent();
                        if (!(!robber || robber.is_alive === 0)) return [3 /*break*/, 58];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.dead, " Bhoot chori nahi kar sakte. Pehle /revive use kar."))];
                    case 57:
                        _l.sent();
                        return [2 /*return*/];
                    case 58: return [4 /*yield*/, db_1.DB_MANAGER.ensureUserExists(env.DB, targetId)];
                    case 59:
                        _l.sent();
                        return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, targetId)];
                    case 60:
                        target = _l.sent();
                        if (!(!target || target.is_alive === 0)) return [3 /*break*/, 62];
                    ...
                    case 62:
                        currentSeconds = Math.floor(Date.now() / 1000);
                        if (!(target.protection_until && target.protection_until > currentSeconds)) return [3 /*break*/, 64];
                        return [4 /*yield*/, sendMessage("".concat(config_1.UI.BORDER_TOP, "\n\u2502 \uD83D\uDEE1\uFE0F <b>A T T A C K   B L O C K E D</b>\n").concat(config_1.UI.BORDER_BOT, "\n\n").concat(config_1.EMOJIS.error, " <b>").concat(targetName, "</b> is under Underworld Protection!\nGuard dogs chased you away."))];
                    case 63:
                        _l.sent();
                        return [2 /*return*/];
                    case 64:
                        if (!(target.balance < 100)) return [3 /*break*/, 66];
                    ...
                    case 70:
                        ...
                        if (!(fineAmount > 0)) return [3 /*break*/, 74];
                    ...
                    case 73:
                        _l.sent();
                        return [3 /*break*/, 76];
                    case 74: return [4 /*yield*/, sendMessage("".concat(config_1.UI.BORDER_TOP, "\n\u2502 \uD83D\uDEA8 <b>B U S T E D !</b>\n").concat(config_1.UI.BORDER_BOT, "\n\n").concat(config_1.EMOJIS.error, " <b>").concat(firstName, "</b> chori karte hue pakda gaya!\n\nLekin jeb khali hone ki wajah se bas pitayi kha ke chhut gaya."))];
                    case 75:
                        _l.sent();
                        _l.label = 76;
                    case 76: return [2 /*return*/];
                    case 77:
                        if (!text.startsWith("/sell")) return [3 /*break*/, 89];
                        if (!(args.length === 0)) return [3 /*break*/, 79];
                    ...
                    case 79:
                        ...
                        if (!!itemData) return [3 /*break*/, 81];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Ye kachra Underworld market me nahi bikta."))];
                    case 80:
                        _l.sent();
                        return [2 /*return*/];
                    case 81: return [4 /*yield*/, db_1.DB_MANAGER.getInventory(env.DB, userId)];
                    case 82:
                        invItems = _l.sent();
                        userItem = invItems.find(function (i) { return i.item_name === itemToSell_1; });
                        if (!(!userItem || userItem.quantity < 1)) return [3 /*break*/, 84];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Tere paas ye item nahi hai. Pehle /inv check kar."))];
                    case 83:
                        _l.sent();
                        return [2 /*return*/];
                    case 84: return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 85:
                        user = _l.sent();
                        if (!user)
                            return [2 /*return*/];
                        currentTime = Math.floor(Date.now() / 1000);
                        sellPrice = muscle_1.MUSCLE.calculateMarketPrice(itemData.base_price, itemData.volatility, currentTime);
                    ...
                    case 88:
                        ...
                        return [2 /*return*/];
                    case 89:
                        if (!(text === "/leaderboard" || text === "/top")) return [3 /*break*/, 92];
                    ...
                    case 91:
                        ...
                        return [2 /*return*/];
                    case 92:
                        if (!(text === "/boss" || text === "/raid")) return [3 /*break*/, 101];
                        return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 93:
                        user = _l.sent();
                        if (!(!user || user.is_alive === 0)) return [3 /*break*/, 95];
                    ...
                    case 95:
                        ...
                        if (!(hit.damage >= bossDefense)) return [3 /*break*/, 97];
                    ...
                    case 102:
                        ...
                        return [2 /*return*/];
                    case 103:
                        if (!(text === "/ping")) return [3 /*break*/, 105];
                    ...
                    case 104:
                        ...
                        return [2 /*return*/];
                    case 105:
                        if (!text.startsWith("/bal")) return [3 /*break*/, 111];
                        targetId = userId;
                    ...
                    case 106:
                        user = _l.sent();
                        if (!user) return [3 /*break*/, 108];
                        return [4 /*yield*/, sendMessage("\uD83D\uDCB0 <b>".concat(targetName, "'s Balance:</b> \u20B9").concat(user.balance))];
                    case 107:
                        _l.sent();
                        return [3 /*break*/, 110];
                    case 108: return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Account not found."))];
                    case 109:
                        _l.sent();
                        _l.label = 110;
                    case 110: return [2 /*return*/];
                    case 111:
                        if (!text.startsWith("/transfer")) return [3 /*break*/, 119];
                        if (userId !== config_1.CONFIG.OWNER_ID)
                            return [2 /*return*/]; // Silent block for non-owners
                        if (!(!update.message.reply_to_message || args.length === 0)) return [3 /*break*/, 113];
                    ...
                    case 117:
                        _l.sent();
                        _l.label = 118;
                    case 118: return [2 /*return*/];
                    case 119:
                        if (!(text.startsWith("/pay") || text.startsWith("/give"))) return [3 /*break*/, 139];
                        if (!!update.message.reply_to_message) return [3 /*break*/, 121];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Kisko paise dene hain? Reply to a player."))];
                    case 120:
                        _l.sent();
                        return [2 /*return*/];
                    case 121:
                        if (!(args.length === 0)) return [3 /*break*/, 123];
                    ...
                    case 123:
                        amount = parseInt(args[0]);
                        if (!(isNaN(amount) || amount <= 0)) return [3 /*break*/, 125];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Sahi amount daal bhai."))];
                    case 124:
                        _l.sent();
                        return [2 /*return*/];
                    case 125:
                        targetId = update.message.reply_to_message.from.id;
                        targetName = update.message.reply_to_message.from.first_name || "Agent";
                        if (!(userId === targetId)) return [3 /*break*/, 127];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Khud ko paise kyu de raha hai?"))];
                    case 126:
                        _l.sent();
                        return [2 /*return*/];
                    case 127:
                        if (!update.message.reply_to_message.from.is_bot) return [3 /*break*/, 129];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Bot moh-maya se door hai."))];
                    case 128:
                        _l.sent();
                        return [2 /*return*/];
                    case 129: return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 130:
                        sender = _l.sent();
                        if (!(!sender || sender.balance < amount)) return [3 /*break*/, 132];
                    ...
                    case 139:
                        if (!(text.startsWith("/defend") || text.startsWith("/safe"))) return [3 /*break*/, 155];
                        return [4 /*yield*/, db_1.DB_MANAGER.getUser(env.DB, userId)];
                    case 140:
                        user = _l.sent();
                        if (!user)
                            return [2 /*return*/];
                        currentSeconds = Math.floor(Date.now() / 1000);
                        if (!(user.protection_until && user.protection_until > currentSeconds)) return [3 /*break*/, 142];
                    ...
                    case 141:
                        ...
                        return [2 /*return*/];
                    case 142:
                        if (!(args.length === 0)) return [3 /*break*/, 144];
                    ...
                    case 144:
                        ...
                        cost = 0;
                        days = 0;
                        if (!(plan === "1d")) return [3 /*break*/, 145];
                        cost = 400;
                        days = 1;
                        return [3 /*break*/, 149];
                    case 145:
                        if (!(plan === "2d")) return [3 /*break*/, 146];
                        cost = 800;
                        days = 2;
                        return [3 /*break*/, 149];
                    case 146:
                        if (!(plan === "3d")) return [3 /*break*/, 147];
                        cost = 1400;
                        days = 3;
                        return [3 /*break*/, 149];
                    case 147: return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Invalid plan (1d/2d/3d)."))];
                    case 148:
                        _l.sent();
                        return [2 /*return*/];
                    case 149:
                        if (!(user.balance < cost)) return [3 /*break*/, 151];
                    ...
                    case 154:
                        ...
                        return [2 /*return*/];
                    case 155:
                        if (!(text === "/upgradedb")) return [3 /*break*/, 162];
                        if (userId !== config_1.CONFIG.OWNER_ID)
                            return [2 /*return*/];
                        _l.label = 156;
                    case 156:
                        _l.trys.push([156, 159, , 161]);
                    ...
                    case 162: return [4 /*yield*/, db_1.DB_MANAGER.getAdmin(env.DB, chatId, userId)];
                    case 163:
                        adminData = _l.sent();
                        isOwner = userId === config_1.CONFIG.OWNER_ID;
                        isHighAdmin = (adminData && adminData.level === 3) || isOwner;
                        if (!text.startsWith("/promote")) return [3 /*break*/, 168];
                        if (!isHighAdmin)
                            return [2 /*return*/];
                        ...
                        if (!!targetId) return [3 /*break*/, 165];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Reply to user or provide an ID."))];
                    case 164:
                        _l.sent();
                        return [2 /*return*/];
                    case 165: return [4 /*yield*/, db_1.DB_MANAGER.setAdmin(env.DB, chatId, targetId, level, "Member")];
                    case 166:
                        _l.sent();
                    ...
                    case 167:
                        ...
                        return [2 /*return*/];
                    case 168:
                        if (!text.startsWith("/demote")) return [3 /*break*/, 171];
                        if (!isHighAdmin)
                            return [2 /*return*/];
                    ...
                    case 170:
                        ...
                        return [2 /*return*/];
                    case 171:
                        if (!text.startsWith("/title")) return [3 /*break*/, 181];
                        if (!isHighAdmin)
                            return [2 /*return*/];
                        ...
                        if (idArg)
                            targetId = parseInt(idArg);
                        if (!(!targetId || !titleName)) return [3 /*break*/, 173];
                        return [4 /*yield*/, sendMessage("".concat(config_1.EMOJIS.error, " Usage: /title [name] [id/reply]"))];
                    case 172:
                        _l.sent();
                        return [2 /*return*/];
                    case 173: return [4 /*yield*/, db_1.DB_MANAGER.setAdmin(env.DB, chatId, targetId, 1, titleName)];
                    case 174:
                        _l.sent();
                        _l.label = 175;
                    case 175:
                        _l.trys.push([175, 178, , 179]);
                    ...
                    case 178:
                        e_2 = _l.sent();
                        return [3 /*break*/, 179];
                    case 179: return [4 /*yield*/, sendMessage("\uD83C\uDFF7\uFE0F <b>TITLE UPDATED!</b>\nTarget <code>".concat(targetId, "</code> is now tagged as: <b>").concat(titleName, "</b>"))];
                    case 180:
                        _l.sent();
                        return [2 /*return*/];
                    case 181:
                        if (!(text === "/admins")) return [3 /*break*/, 184];
                    ...
                    case 182:
                        ...
                        return [4 /*yield*/, sendMessage(list + config_1.UI.BORDER_BOT)];
                    case 183:
                        _l.sent();
                        return [2 /*return*/];
